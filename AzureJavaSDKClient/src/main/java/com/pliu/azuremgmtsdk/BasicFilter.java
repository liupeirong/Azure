...
public class BasicFilter implements Filter {
    ...
    private static String apiEndpoint = "https://management.core.windows.net/";
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (request instanceof HttpServletRequest) {
            ...
            try {
            	String subscriptionId = httpRequest.getParameter("subscriptionId");
            	if (subscriptionId != null)
            		httpRequest.getSession().setAttribute("subscriptionId", subscriptionId);
                ...
                if (!AuthHelper.isAuthenticated(httpRequest)) {
                    if (AuthHelper.containsAuthenticationData(httpRequest)) {
                        ...
                        // not authenticated
                    	// we can get the tenant id for the subscription first
                        getTenantIdForSubscription(httpRequest);
    ...
    private AuthenticationResult getAccessTokenFromRefreshToken(String refreshToken) throws Throwable {
        ...
        try {
            ...
            context = new AuthenticationContext(authority + userTenant + "/", true,
                    service);
        ...
        try {
            ...
            Future<AuthenticationResult> future = context
                    .acquireTokenByAuthorizationCode(authCode, new URI(
                            currentUri), credential, apiEndpoint, null);
            result = future.get();
        } catch (ExecutionException e) {
            throw e.getCause();
        } finally {
    ...
    private String getRedirectUrl(String currentUri, String state, String nonce)
        ...
        String redirectUrl = authority
                ...
                + clientId + "&resource=" + URLEncoder.encode(apiEndpoint, "UTF-8")
                //+ "&prompt=consent"
                + "&state=" + state
                + "&nonce=" + nonce;
