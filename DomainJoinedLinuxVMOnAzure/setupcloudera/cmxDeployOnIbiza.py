...
def getParameterValue(vmsize, parameter):
    ...
    defaultSwitcher = {
        ...
        "impalad_memory_limit": "3800000000"
    }
    if not (vmsize.startswith("Standard_DS14") or vmsize.startswith("Standard_DS13")):
        return defaultSwitcher.get(parameter, "0")
...
def getDataDiskCount():
    ...
    toconnect=socket.getfqdn(cmx.cm_server).replace("0", "1") #pliu, change made for VM naming conventions as prefix0,prefix1,prefix2... with prefix0 being the master node
    log("connecting to: " + toconnect)
...
def setZookeeperOwnerDir(HA):
    ...
    if HA:
        client=SSHClient()
        client.set_missing_host_key_policy(paramiko.client.AutoAddPolicy())
        toconnect=socket.getfqdn(cmx.cm_server).replace("-mn0", "-mn1") #pliu, this needs to change if you provision HA
        client.connect(toconnect, username=cmx.ssh_root_user, password=cmx.ssh_root_password)
        client.exec_command("sudo chown zookeeper:zookeeper "+LOG_DIR+"/zookeeper")
        toconnect=socket.getfqdn(cmx.cm_server).replace("-mn0", "-mn2") #pliu, this needs to change if you provision HA
        client.connect(toconnect, username=cmx.ssh_root_user, password=cmx.ssh_root_password)
        client.exec_command("sudo chown zookeeper:zookeeper "+LOG_DIR+"/zookeeper")
